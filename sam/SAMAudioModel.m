                               ...
                               AudioBufferList *ioData)
{
    ...
    TREAudioModel* this = (__bridge TREAudioModel *)inRefCon;
...
{
    static TREAudioModel *sharedAudioModel = nil;
    static dispatch_once_t onceToken;

    dispatch_once(&onceToken, ^{
        sharedAudioModel = [[TREAudioModel alloc] init];
    });
...
- (id)init
{
    ...
    if (self)
    {
        ...
        [self setupAudioUnit];
        OSErr err = AudioUnitInitialize(treUnit);
...
- (void)dealloc
{
    if (treUnit)
    {
        AudioOutputUnitStop(treUnit);
        AudioUnitUninitialize(treUnit);
        AudioComponentInstanceDispose(treUnit);
        treUnit = nil;
    }
...
- (void)setupAudioUnit
{
    ...
    OSErr err = AudioComponentInstanceNew(defaultOutput, &treUnit);
    NSAssert1(treUnit, @"Error creating unit: %ld", err);
    ...
    UInt32 flag = 1;
    err = AudioUnitSetProperty(treUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Output, 0, &flag, sizeof(flag));
    ...
    err = AudioUnitSetProperty(treUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &streamFormat, sizeof(AudioStreamBasicDescription));
    ...
    //
    err = AudioUnitSetProperty(treUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Global, 0, &input, sizeof(input));
...
- (void)startAudioSession
{
    ...
    // Start playback
    OSErr err = AudioOutputUnitStart(treUnit);
